function grid=MGSimulFFTseed(x,y,z,mu,sigma2,m,lx,ly,lz,seed)

%grid=MGSimulFFTseed(x,y,z,mu,sigma2,m,lx,ly,lz,seed)
% See DS user guide for functions documentation

% Written by Gregoire Mariethoz, 2010
% Modified by Inigo Irarrazaval, 2015
...
xc=(xx(1,end,1)-xx(1,1,1))./2; % coordinates of the center of the grid
yc=(yy(end,1,1)-yy(1,1,1))./2;
zc=(zz(1,1,end)-zz(1,1,1))./2;
...
ran=sqrt(grid).*exp(1i*angle(fftn(seed))); 
